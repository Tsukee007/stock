...
export async function POST(req: Request) {
  try {
    const { prenom, email, interet, consent_email, consent_rgpd } = await req.json()
    ...
    const { error } = await supabase
      .from('waitlist')
      .insert({ prenom, email, interet, consent_email, consent_rgpd })
    ...
    // Email de confirmation a l'inscrit
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 520px; margin: 0 auto; padding: 32px 24px;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin-top: 16px;">
            ...
            <tr style="background: #f9fafb;"><td style="padding: 8px; color: #6b7280; font-size: 14px;">Consent email</td><td style="padding: 8px; font-weight: 500; color: #111827;">${consent_email ? 'Oui' : 'Non'}</td></tr>
          </table>
        </div>
      `
    })
